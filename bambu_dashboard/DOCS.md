...
- **Home Assistant OS** ou **HA Supervised** (le mode Container n'a pas
  le Supervisor donc ne supporte pas les add-ons)
- **Imprimante Bambu Lab** avec AMS, accessible en réseau local depuis HA
- **Mode LAN activé** sur l'imprimante (voir plus bas)
...
**Important** : activer **Mode LAN uniquement** (ou LAN + Cloud) dans
Réglages → Réseau. Sans ça, le broker MQTT local est désactivé et
l'add-on ne peut pas se connecter.
...
1. Créer une automation dans HA qui écoute un webhook :
   ```yaml
   trigger:
     - platform: webhook
   ...
   ```
2. Dans les options de l'add-on, coller l'URL du webhook :
   ```
...
1. Après avoir démarré l'add-on, ouvre l'interface Web via
   **Ouvrir l'interface Web** ou le panneau latéral **Bambu**
2. L'onglet **AMS** affiche l'état courant des 4 slots dès le premier
   message MQTT reçu (quelques secondes)
3. L'onglet **Inventaire** est vide — il se remplit automatiquement au
   fur et à mesure que les bobines sont détectées dans l'AMS
...
1. Onglet **Inventaire** → bouton **Scan Flipper**
...
**Navigateur requis** : Chrome ou Edge — Web Serial n'est pas supporté
par Firefox/Safari.

Voir [`flipper/bambu_scanner/README.md`](../flipper/bambu_scanner/README.md)
pour la construction et l'installation du plugin Flipper.

## Overlay frontend (itération rapide)

L'add-on supporte un overlay du frontend via `/share/bambu_dashboard/frontend/`
pour modifier `index.html` **sans rebuild de l'image Docker** :

1. Copie `frontend/index.html` de l'image dans
   `/share/bambu_dashboard/frontend/index.html`
2. Modifie-le librement
3. **Redémarre l'add-on** — les modifs sont appliquées

Supprime le fichier pour revenir à la version embarquée.

## Persistance des données

- La base SQLite est dans `/data/bambu.db` (persistée par HA entre
  redémarrages de l'add-on et mises à jour)
- Mode WAL activé (performances + lectures concurrentes)
- Les logs de consommation sont append-only, filtrés par seuil de 2 %
  pour éviter le bruit MQTT
...
- Vérifie le `printer_access_code` (attention aux caractères)
- Regarde les logs de l'add-on : erreurs TLS ou auth = code ou S/N faux

### "Failed to open serial port" au scan Flipper

- **Ferme qFlipper, PuTTY, Arduino IDE, tout moniteur série** qui pourrait
  tenir le port COM ouvert (Windows → ports COM exclusifs)
...
- Ouvre la console Chrome DevTools (F12) et regarde l'onglet Console
  pour les erreurs `BAMBU_NFC parse error`
- Vérifie que tu es bien en **HTTPS** sur HA (l'ingress HA est HTTPS par
  défaut — Web Serial exige une origine sécurisée)

### Les notifications stock bas ne partent pas

- Vérifie que `ha_webhook_url` est rempli
- Teste le webhook manuellement avec curl
- Les notifs sont émises **une seule fois par franchissement** du seuil
  (pour éviter le spam MQTT) — change de bobine pour retester

## Liens

- Dépôt : https://code.e-odyssey.net/coom/bambuboard
- Plugin Flipper : [`flipper/bambu_scanner/`](../flipper/bambu_scanner/)
